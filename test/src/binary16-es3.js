...
describe('Binary16 numbers', function() {     
    ...
    it('unpack -0', function() {
        assert.equal(
        	ieee.unpack([0,128],  0, 5, 11), 
            -0);
    });

    it('pack 1e-25', function() {
        var buffer = [];
        ieee.pack(buffer, 0, 1e-25, 5, 11);
        assert.deepEqual(
            buffer, 
            [0,0]);
    });
    ...
    it('pack NaN', function() {
        var buffer = [];
        ieee.pack(buffer, 0, NaN, 5, 11);
        assert.deepEqual(
            buffer, 
            [0, 126]); // Python struct.pack('e', math.nan)
    });
    it('unpack NaN', function() {
        assert.ok(isNaN(ieee.unpack([0, 126],  0, 5, 11)));
    });

    // Infinity
    it('pack Infinity', function() {
        var buffer = [];
        ieee.pack(buffer, 0, Infinity, 5, 11);
        assert.deepEqual(
        	buffer, 
    ...
    it('unpack -Infinity', function() {
        assert.equal(
        	ieee.unpack([0x00, 0xfc],  0, 5, 11), 
            -Infinity);
    });
    // round to Infinity
    it('round 65520 to Infinity when packing', function() {
        var buffer = [];
        ieee.pack(buffer, 0, 65520, 5, 11);
        assert.deepEqual(
            buffer, 
            [0, 124]);
    });
    ...
    it('pack pi as 3.14159', function() {
        var buffer = [];
        ieee.pack(buffer, 0, 3.14159, 5, 11);
    ...
    it('unpack pi as 3.140625', function() {
        ...
        assert.equal(ieee.unpack([0x48,0x42],  0, 5, 11).toFixed(6), 
            '3.140625');
    });
    ...
    it('unpack 1', function() {
        assert.equal(
        	ieee.unpack([0x00,0x3c],  0, 5, 11), 
            1);
    });
    it('pack -2048 (min exact)', function() {
        var buffer = [];
        ieee.pack(buffer, 0, -2048, 5, 11);
        assert.deepEqual(
        	buffer, 
            [0x00,0xe8]);
    });
    ...
    it('unpack 2050', function() {
        assert.deepEqual(
            ieee.unpack([0x01,0x68],  0, 5, 11), 
            2050);
    });
    it('pack 2051 like it pack 2050', function() {
        var buffer = [];
        ieee.pack(buffer, 0, 2051, 5, 11);
        assert.deepEqual(
            buffer, 
            [0x01,0x68]);
    });
